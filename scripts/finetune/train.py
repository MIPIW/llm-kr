...
m.add_argument(
    ...
    type=str,
    required=True,
    choices=["llama3-8b", "gemma2-9b", "qwen2-7b"],
    help="IT model type (required)",
)
m.add_argument(
    "--train_dataset",
    type=str,
    default="/node_storage2/data_llm_kr/data_it_train_240724.csv",
    help="train dataset (should be in format of csv)",
)
m.add_argument(
    "--eval_dataset",
    type=str,
    default="/node_storage2/data_llm_kr/data_it_eval_240724.csv",
    help="eval dataset (should be in format of csv)",
)
m.add_argument(
    "--output_dir",
    type=str,
...
h.add_argument(
    "--warmup_ratio", type=float, default=0.0, help="warmup ratio of learning rate"
)
h.add_argument(
...
l.add_argument(
    ...
    default=None,
    choices=["lora", "dora"],
    help="whether to use LoRA or DoRA; defaults to None",
)
l.add_argument("--lora_rank", type=int, default=8, help="rank for LoRA")
l.add_argument("--lora_alpha", type=int, default=32, help="lora_alpha for LoRA")
l.add_argument(
    "--lora_dropout", type=float, default=0.1, help="dropout probability for LoRA"
)
l.add_argument(
    "--lora_target",
    type=str,
    default="all",
    choices=["all", "no_qk", "no_v", "no_qkv"],
    help="target modules to apply LoRA; defaults to 'all'",
)
...
s.add_argument("--save_steps", type=int, default=500, help="save steps")
s.add_argument(
    "--save_total_limit", type=int, default=1, help="# of last checkpoints to save"
)
s.add_argument(
...
e.add_argument("--eval_steps", type=int, default=500, help="eval steps")
e.add_argument(
    "--eval_accumulation_steps", type=int, default=None, help="eval accumulation steps"
)
...
o.add_argument("--num_proc", type=int, default=8, help="# of processors to be used")
o.add_argument(
    "--seed", type=int, default=42, help="random seed for random, numpy, torch"
)
>>>>>>> 3451a07 (directory refined)
...
filer = logging.FileHandler(
    filename=f"{output_dir}/train_log.log", mode="w", encoding="utf-8"
)
filer.setFormatter(formatter)
filer.setLevel(logging.DEBUG)
logger.addHandler(filer)
...
=======
    ...
    if tokenizer.pad_token is None:
        tokenizer.add_special_tokens(
            {"pad_token": "<pad>"}
        )  # Llama3 doesn't have pad_token

    # flash attention 2 only for Gemma
    attn_implementation = (
        "eager" if model_path == "google/gemma-2-9b-it" else "flash_attention_2"
    )
>>>>>>> 3451a07 (directory refined)
...
=======
    ...
    if args.save_strategy == "no":
        logger.warning(
            "Argument 'save_strategy' is set to 'no', make sure your model to be saved separately after training"
        )
>>>>>>> 3451a07 (directory refined)
    else:
...
=======
    ...
    if args.save_strategy == "no":
        logger.warning(
            "Argument 'eval_strategy' is set to 'no', no evaluation will be done during training"
        )
    else:
        sft_config_dict["eval_steps"] = args.eval_steps
        sft_config_dict["eval_accumulation_steps"] = args.eval_accumulation_steps
    ...
    if args.save_strategy == "no":
        logger.warning(
            "Argument 'logging_strategy' is set to 'no', no log will be documented"
        )
    else:
        if args.logging_strategy == "steps":
            sft_config_dict["logging_steps"] = args.logging_steps
